...
let currentAudio = null;  // Objeto de audio actual en reproducción
let currentButton = null; // Botón de play/pause actual

function showPlayButton(id) {
    document.getElementById(`play-btn-${id}`).style.display = 'block';
}
...
function togglePlayPause(id, audioUrl) {
    const playButtonIcon = document.getElementById(`icon-${id}`);
    const playIcon = "{% static 'assets/img/icons/play.svg' %}";
    const pauseIcon = "{% static 'assets/img/icons/pause.svg' %}";

    // Si se está reproduciendo otra canción, detenerla
    if (currentAudio && currentAudio.id !== id) {
        currentAudio.audio.pause();
        currentAudio.buttonIcon.src = playIcon;
        currentAudio = null;
        currentButton = null;
    }

    // Si no hay canción en reproducción o la misma se ha pausado
    if (!currentAudio || currentAudio.id !== id) {
        // Crear nuevo objeto de audio si es necesario
        if (!currentAudio || currentAudio.id !== id) {
            currentAudio = { id: id, audio: new Audio(audioUrl), buttonIcon: playButtonIcon };
        }
        
        // Reproducir la canción y cambiar ícono a pausa
        currentAudio.audio.play();
        playButtonIcon.src = pauseIcon;
        
        // Configurar el audio para que al terminar, regrese al ícono de play
        currentAudio.audio.onended = () => {
            playButtonIcon.src = playIcon;
            currentAudio = null;
        };
    } else if (!currentAudio.audio.paused) {
        // Si la canción ya está sonando, pausarla y cambiar ícono a play
        currentAudio.audio.pause();
        playButtonIcon.src = playIcon;
        currentAudio = null;
    }
